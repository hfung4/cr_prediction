...
def test_snake_case_column_names(input_columns: list[str], expected_columns: list[str]):
    ...
    schema = T.StructType(
        [T.StructField(c, T.StringType(), True) for c in input_columns]
    )
...
def input_dataframe_for_map_column() -> PySparkDataFrame:
    ...
    """
    data = [("James", "Strongly Agree"), ("Michael", "Disagree"), ("Anna", None)]
...
def test_map_column(input_dataframe_for_map_column):
    ...
    assert (
        type(input_dataframe_for_map_column) == PySparkDataFrame
    ), "Input must be PySparkDataFrame"
    ...
    map_result_list = (
        input_dataframe_for_map_column.transform(
            lambda _df: map_column(_df, test_column, test_map)
        )
        .select(test_column)
        .collect()
    )
    ...
    assert [
        x[test_column] for x in map_result_list
    ] == expected_result, "map_column function failed"
    ...
    map_result_list = (
        input_dataframe_for_map_column.transform(
            lambda _df: map_column(_df, test_column, test_map, new_column)
        )
        .select(new_column)
        .collect()
    )
    ...
    assert [
        x[new_column] for x in map_result_list
    ] == expected_result, "map_column function failed"
...
@pytest.mark.processing_utils
def test_add_social_security_retirement_age(
    input_dataframe_for_social_security_retirement_age,
):
    """
    ...
    # Test data type of input
    assert (
        type(input_dataframe_for_social_security_retirement_age) == PySparkDataFrame
    ), "Input must be PySparkDataFrame"

    # Test a function on mock data

    retirement_ages = (
        input_dataframe_for_social_security_retirement_age.transform(
            add_social_security_retirement_age
        )
        .select("socsec_full_retire_age")
        .collect()
    )
    ...
    assert [
        x["socsec_full_retire_age"] for x in retirement_ages
    ] == expectation, "add_social_security_retirement_age function failed"
...
@pytest.mark.processing_utils
def test_transform_YEARSMEMBER_feature(
    input_dataframe_for_transform_YEARSMEMBER_feature,
):
    """
    ...
    # Test data type of input
    assert (
        type(input_dataframe_for_transform_YEARSMEMBER_feature) == PySparkDataFrame
    ), "Input must be PySparkDataFrame"

    # Test a function on mock data
    YEARSMEMBERs = [
        x.YEARSMEMBER
        for x in input_dataframe_for_transform_YEARSMEMBER_feature.transform(
            transform_YEARSMEMBER_feature, impute_yearsmember=20, max_age=99
        )
        .select("YEARSMEMBER")
        .collect()
    ]

    EXPECTED_YEARSMEMBERs = [
        x.EXPECTED_YEARSMEMBER
        for x in input_dataframe_for_transform_YEARSMEMBER_feature.select(
            "EXPECTED_YEARSMEMBER"
        ).collect()
    ]

    assert (
        YEARSMEMBERs == EXPECTED_YEARSMEMBERs
    ), "transform_YEARSMEMBER_feature function failed"


@pytest.fixture
def input_dataframe_for_add_tot_AUM_per_yrmbr_feature() -> PySparkDataFrame:

    data = ([None, 13.0, 0.0], [1.0, 13.0, 13.0], [0.25, 13.0, 26.0])
...
@pytest.mark.processing_utils
def test_add_tot_AUM_per_yrmbr_feature(
    input_dataframe_for_add_tot_AUM_per_yrmbr_feature,
):
    """
    ...
    # Test data type of input
    assert (
        type(input_dataframe_for_add_tot_AUM_per_yrmbr_feature) == PySparkDataFrame
    ), "Input must be PySparkDataFrame"

    # Test a function on mock data
    tot_AUM_per_yrmbr = [
        x.tot_AUM_per_yrmbr
        for x in input_dataframe_for_add_tot_AUM_per_yrmbr_feature.transform(
            add_tot_AUM_per_yrmbr_feature
        )
        .select("tot_AUM_per_yrmbr")
        .collect()
    ]

    EXPECTED_tot_AUM_per_yrmbr = [
        x.EXPECTED_tot_AUM_per_yrmbr
        for x in input_dataframe_for_add_tot_AUM_per_yrmbr_feature.select(
            "EXPECTED_tot_AUM_per_yrmbr"
        ).collect()
    ]

    assert (
        tot_AUM_per_yrmbr == EXPECTED_tot_AUM_per_yrmbr
    ), "add_tot_AUM_per_yrmbr_feature function failed"
...
def input_dataframe_for_resample_responder_df() -> PySparkDataFrame:
    ...
    schema = T.StructType([T.StructField("Target", T.IntegerType(), True)])
...
def test_resample_responder_df(input_dataframe_for_resample_responder_df):
    ...
    # Test data type of input
    assert (
        type(input_dataframe_for_resample_responder_df) == PySparkDataFrame
    ), "Input must be PySparkDataFrame"
    ...
    resampled_df = (
        input_dataframe_for_resample_responder_df.transform(
            resample_responder_df, non_responder_ratio
        )
        .groupBy("Target")
        .count()
    )

    responder_count = resampled_df.filter(resampled_df.Target == 1).collect()[0][
        "count"
    ]
    non_responder_count = resampled_df.filter(resampled_df.Target == 0).collect()[0][
        "count"
    ]

    EXPECTED_responder_count = input_dataframe_for_resample_responder_df.filter(
        input_dataframe_for_resample_responder_df.Target == 1
    ).count()
    EXPECTED_non_responder_count = non_responder_ratio * EXPECTED_responder_count

    assert (responder_count == EXPECTED_responder_count) and (
        pytest.approx(non_responder_count, 1) == EXPECTED_non_responder_count
    ), "resample_responder_df function failed"
...
def test_parse_dates():
    ...
    with pytest.raises(
        ValueError,
        match="Invalid date format for '2023/01/01'. Expected format is YYYY-MM-DD.",
    ):
    ...
    with pytest.raises(
        ValueError,
        match="Invalid date format for '2023/02/01'. Expected format is YYYY-MM-DD.",
    ):
        parse_dates(date_input)
    ...
    with pytest.raises(
        ValueError,
        match="Invalid date format for 'invalid-date'. Expected format is YYYY-MM-DD.",
    ):
        parse_dates(date_input)
...
def test_check_ordering_of_train_test_periods():
    ...
    try:
        ...
        pytest.fail(
            "check_ordering_of_train_test_periods raised ValueError unexpectedly!"
        )
    ...
    with pytest.raises(
        ValueError,
        match="Time periods for testing cannot precede time periods for training.",
    ):
        check_ordering_of_train_test_periods(train_start_dates, test_start_dates)
...
def test_check_consistent_time_period_cadences():
    ...
    try:
        ...
        pytest.fail(
            "check_consistent_time_period_cadences raised ValueError unexpectedly!"
        )
    ...
    with pytest.raises(
        ValueError,
        match="Irregular cadence detected: the difference between the longest and shortest period lengths must not exceed 5 days.",
    ):
        check_consistent_time_period_cadences(period_start_dates)

    # Test with less than 3 dates (should not raise an error)
    period_start_dates = [datetime.date(2023, 1, 1), datetime.date(2023, 2, 1)]
    try:
        check_consistent_time_period_cadences(period_start_dates)
    except ValueError:
        pytest.fail(
...
def test_get_validated_time_period_feature_pipeline():
    ...
    ), "get_validated_time_period_feature_pipeline function failed with valid input"
...
def test_get_validated_time_period_inference_pipeline():
    ...
    expected_result = {
        "serving_start_dates": [date(2023, 1, 1), date(2023, 2, 1), date(2023, 3, 1)]
    }
    ...
    time_period_dict = {"serving_start_dates": []}

    expected_result = {"serving_start_dates": []}
...
def test_get_closest_date_mapping():
    ...
    test_cases = [
        # Test case 1: Simple case with distinct dates
        {
            ...
            "expected": {"2023-01-01": "2023-01-15", "2023-02-01": "2023-02-15"},
        },
        # Test case 2: Multiple time periods mapping to the same effective date
        {
            ...
            "expected": {"2023-01-01": "2023-01-15", "2023-02-01": "2023-01-20"},
        },
        ...
        # Test case 5: Effective dates with no close match
        {
            ...
            "expected": {"2023-02-01": "2023-12-15"},
        },
    ]
...
def input_dataframe_for_create_adjusted_effective_start_date() -> PySparkDataFrame:
    ...
    schema = T.StructType([T.StructField("effective_start_date", T.StringType(), True)])
...
def test_filter_after_a_given_date():
    ...
    with pytest.raises(TypeError):
        filter_after_a_given_date(
            df.withColumn("date_col", F.col("date_col").cast("string")),
            "date_col",
            "2023-01-01",
        )
...
def input_dataframe_for_create_adjusted_effective_start_date() -> PySparkDataFrame:
    ...
    schema = T.StructType([T.StructField("effective_date", T.StringType(), True)])

    return spark.createDataFrame(data=data, schema=schema)


@pytest.mark.processing_utils
def test_create_adjusted_effective_start_date(
    input_dataframe_for_create_adjusted_effective_start_date,
):
    """
    ...
    # Test data type of input
    assert (
        type(input_dataframe_for_create_adjusted_effective_start_date)
        == PySparkDataFrame
    ), "Input must be PySparkDataFrame"
    ...
    result_df = input_dataframe_for_create_adjusted_effective_start_date.transform(
        lambda _df: create_adjusted_effective_start_date(
            _df, "effective_date", time_periods
        )
    )
    ...
    ), "create_adjusted_effective_start_date function failed"
...
def test_get_time_periods_label_mapping_feature_pipeline():
    ...
    time_period_dict = {
        ...
        "test_start_dates": [datetime.date(2023, 4, 1), datetime.date(2023, 5, 1)],
    }
    ...
    assert (
        result_mapping == expected_mapping
    ), "The function _get_time_periods_label_mapping_feature_pipeline did not return the expected mapping."
...
@pytest.mark.processing_utils
def test_create_set_type_for_train_test_data(
    input_dataframe_for_create_set_type_for_train_test_data,
):
    """
    ...
    # Test data type of input
    assert (
        type(input_dataframe_for_create_set_type_for_train_test_data)
        == PySparkDataFrame
    ), "Input must be PySparkDataFrame"
    ...
    result_df = input_dataframe_for_create_set_type_for_train_test_data.transform(
        lambda _df: create_set_type_for_train_test_data(
            _df, "adjusted_effective_date", time_period_dict
        )
    )
    ...
    ), "create_set_type_for_train_test_data function failed"
...
def test_convert_decimal_to_double():
    ...
    for column in processed_df.columns:
        assert (
            processed_df.schema[column].dataType == DoubleType()
        ), f"{column} was not converted to DoubleType"
    ...
    for row, expected in zip(result_data, expected_data):
        assert (
            row[0] == expected[0] and row[1] == expected[1]
        ), f"Row {row} does not match expected {expected}"
...
def test_convert_integer_to_double():
    ...
    for column in processed_df.columns:
        if column != config.model.OUTCOME_VARIABLE:  # Exclude the outcome variable
            ...
            assert (
                processed_df.schema[column].dataType == IntegerType()
            ), f"{column} should remain as IntegerType"
