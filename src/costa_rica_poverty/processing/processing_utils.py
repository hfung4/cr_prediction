...
def transform_and_add_membership_tenure_features(
    df: PySparkDataFrame, impute_yearsmember: int = 25, max_age: int = 99
) -> PySparkDataFrame:
    """
        ...
        .withColumn(
            ...
            F.when(
                ...
                F.array_max(
                    F.array(
                        F.lit(0),
                        F.datediff(F.col("eff_beg_dt"), F.col("MBRDT")) / F.lit(365.25),
                    )
                ),
            ).otherwise(F.col("YEARSMEMBER")),
        )
...
def input_args_time_periods_validation(time_periods: Dict[str, List[str]]) -> None:
    ...
    if time_periods["serving_start_dates"] and (
        time_periods["train_start_dates"] is not None
        or time_periods["test_start_dates"] is not None
    ):
        raise ValueError(
            "You cannot provide --serving_start_dates together with --train_start_dates or --test_start_dates."
        )
    elif not (
        (time_periods["train_start_dates"] and time_periods["test_start_dates"])
        or time_periods["serving_start_dates"]
    ):
        raise ValueError(
...
def create_adjusted_effective_start_date(
    ...
    unique_effective_start_dates = {
        datetime.strptime(date_str, "%Y-%m-%d").date() for date_str in unique_dates_str
    }
