...
class GreatExpectationsContext:
    ...
    def add_suite(self, suite_name: str) -> None:
        ...
        self.expectation_suite = gx.ExpectationSuite(name=suite_name)
...
def validate_gold_data(
    df: PySparkDataFrame, pipeline_type: str
    ...
    # get expected schema
    expected_schema = (
        gold_expectations["TRAIN_SCHEMA"]
        if pipeline_type == "feature"
        else gold_expectations["SERVING_SCHEMA"]
    )
    ...
    if pipeline_type == "feature":
        ...
        # validate the expecation suite
        missingness_in_response_validation = gx_context.validate_suite(
            "missingness_check"
        )
    ...
    expected_categorical_options = (
        gold_expectations["TRAIN_CATEGORICAL_OPTIONS"]
        if pipeline_type == "feature"
        else gold_expectations["SERVING_CATEGORICAL_OPTIONS"]
    )
    ...
    for column, expected_type in expected_categorical_options.items():
        gx_context.suites["categorical_options"].add_expectation(
            gx.expectations.ExpectColumnDistinctValuesToBeInSet(
                column=column, value_set=expected_type
            )
        )
    ...
    result = Result(
        ...
        (
            True
            if pipeline_type != "feature"
            else missingness_in_response_validation.success
        ),
