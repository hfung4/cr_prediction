...
def get_latest_best_run(
    experiment_id: str, parent_run_filter_string: str = ""
) -> Dict[str, object]:
    ...
    return {"best_run_id": best_run_id, "best_run": mlflow.get_run(best_run_id)}
...
def get_overall_best_run(
    ...
    best_run_id = mlflow.search_runs(
        ...
        order_by=["metrics.median_cv_log_loss ASC"],
    ).run_id[0]
...
def get_comparison_metrics(
    run: object, metric_names: List[str], larger_is_better: List[bool]
) -> Dict[str, float]:
    """
    ...
    adjusted_metrics = {
        metric_name: (x if flag else -x)
        for metric_name, x, flag in zip(metric_names, metrics, larger_is_better)
    }
